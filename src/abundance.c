...
double ngal_triax(const Model *model, double r, double c, double z, double obs_type)
{
   ...
   sum1 = 0.0;
   for(i=0;i<N;i++){
      logM1 = LNMH_MIN + dlogM*(double)i;
      R1 = r_vir(model, exp(logM1), c, z);
      sum2 = 0.0;
      for(j=0;j<N;j++){
         logM2 = LNMH_MIN + dlogM*(double)j;
         R2 = r_vir(model, exp(logM2), c, z);
         x = r/(R1 + R2);
         ...
         if (y<0) {
            ...
            sum2 +=  int_for_ngal_den(logM2, (void*)&p);
         } else {
            P  = (3.0 - 2.0*y)*y*y;
            sum2 +=  int_for_ngal_den(logM2, (void*)&p)*P;
         }
      }
      sum1 +=  int_for_ngal_den(logM1,(void*)&p)*sum2*dlogM;
   }
